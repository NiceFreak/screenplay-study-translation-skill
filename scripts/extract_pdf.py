...
def text_rows(pdf_path: Path, displayed_page_offset: int) -> list[dict[str, Any]]:
    ...
    for page_number, _page_obj, content_id in scan_markers.page_content_ids(objects):
        font_styles = scan_markers.page_font_styles(objects, _page_obj)
        ...
        for op in scan_markers.iter_text_ops(data):
            ...
            x = float(op["x"])
            y = float(op["y"])
            key = (page_number, text, round(x), round(y))
            if key in seen:
                continue
            seen.add(key)
            style_spans: list[dict[str, Any]] = []
            styles = set()
            font_resource = op.get("font_resource")
            if isinstance(font_resource, str) and font_resource in font_styles:
                styles.update(font_styles[font_resource])
            if styles and text:
                style_spans.append(
                    {
                        "start": 0,
            ...
            rows.append(
                {
                    ...
                    "zone": "raw",
                    **({"style_spans": style_spans} if style_spans else {}),
                }
            )
    return sorted(
...
def join_row_group(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in ordered:
        ...
        parts.append(text)
        cursor += len(text)
        for span in row.get("style_spans", []) or []:
            ...
            clean_styles = sorted(
                {
                    str(style)
                    for style in styles
                    if str(style) in {"bold", "italic", "underline"}
                }
            )
            if clean_styles:
                style_spans.append(
    ...
    coalesced_spans = coalesce_style_spans(style_spans)
    if coalesced_spans:
        line["style_spans"] = coalesced_spans
    line["zone"] = line_zone(line)
    return line


def coalesce_style_spans(spans: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if not spans:
        return []
    ordered = sorted(
        spans,
    ...
    for span in ordered:
        current = {
            ...
            "end": int(span["end"]),
            "styles": list(span["styles"]),
        }
        if (
            coalesced
            and coalesced[-1]["end"] == current["start"]
            and coalesced[-1]["styles"] == current["styles"]
        ):
            coalesced[-1]["end"] = current["end"]
        else:
...
def write_lines(
    ...
    payload = {
        ...
        "assumptions": {
            ...
            "style_spans": "bold/italic/underline ranges only; font resource names are not exported",
...
def missing_display_pages(pages: list[int]) -> list[int]:
    if not pages:
        return []
    return [page for page in range(pages[0], pages[-1] + 1) if page not in pages]
...
def main() -> int:
    ...
    if pages:
        print(
            f"INFO text_pages.display count={len(pages)} pages={compact_ranges(pages)}"
        )
        missing_pages = missing_display_pages(pages)
        if missing_pages:
            print(
                f"WARN text_pages.display_missing pages={compact_ranges(missing_pages)}"
            )
    return 0
