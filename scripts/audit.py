...
def html_display_pages(html: str) -> list[int]:
    pages = {
        int(value)
        for value in re.findall(r"\bdata-display-page=[\"'](-?\d+)[\"']", html)
    }
    return sorted(pages)


def expected_body_pages(
    display_page_start: int | None, display_page_end: int | None
) -> list[int]:
    if display_page_start is None and display_page_end is None:
        return []
    start = max(1, display_page_start if display_page_start is not None else 1)
    end = display_page_end if display_page_end is not None else start
    if end < start:
        return []
...
def check_html(
    ...
    display_pages = html_display_pages(text)
    if display_pages:
        findings.append(
            Finding(
                "INFO",
                "html.display_pages",
                f"count={len(display_pages)} pages={compact_ranges(display_pages)}",
            )
        )
        expected_pages = expected_body_pages(display_page_start, display_page_end)
        if expected_pages:
            missing_pages = sorted(set(expected_pages) - set(display_pages))
            if missing_pages:
                findings.append(
                    Finding(
                        "FAIL",
                        "html.display_pages_missing",
                        f"pages={compact_ranges(missing_pages)}",
                    )
                )
    else:
        findings.append(
            Finding(
                "WARN",
                "html.display_pages",
                "no data-display-page attributes found",
            )
        )
...
def run(
    ...
    findings.extend(
        check_html(project_file, config, html, display_page_start, display_page_end)
    )
